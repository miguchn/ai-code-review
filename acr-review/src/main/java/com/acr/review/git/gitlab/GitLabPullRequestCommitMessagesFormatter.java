...
/** 将 GitLab MR commits API 响应格式化为审查上下文文本。 */
final class GitLabPullRequestCommitMessagesFormatter
{
    static final int MAX_COMMITS = 30;
    static final int MAX_LINE_LENGTH = 500;

    private GitLabPullRequestCommitMessagesFormatter()
    {
    }
    ...
    static String firstLine(JSONObject commitNode)
    {
        ...
        String title = commitNode.getString("title");
        if (title != null && !title.isBlank())
        {
            return trimLine(title);
        }
        String message = commitNode.getString("message");
        if (message == null || message.isBlank())
        {
            return "";
        }
        int newline = message.indexOf('\n');
        return trimLine(newline >= 0 ? message.substring(0, newline) : message);
    }
