...
/** Gitee Issue/PR 总结评论适配（list / create / update）。 */
@Component
public class GiteePullRequestCommentClient implements GitPullRequestCommentClient
{
    private static final MediaType JSON_MEDIA = MediaType.parse("application/json; charset=utf-8");
    private static final Pattern TOKEN_PATTERN = Pattern.compile("[A-Za-z0-9_]{16,}");
    ...
    GiteePullRequestCommentClient(HttpUrl apiBaseUrl, int connectTimeoutMs, int readTimeoutMs)
    {
                                                                 ...
                                                                 String marker)
    {
        ...
        {
            HttpUrl url = issueCommentsUrl(repository, prNumber, token)
                .newBuilder()
                .addQueryParameter("page", String.valueOf(page))
                .addQueryParameter("per_page", String.valueOf(ReviewDeliveryConstants.COMMENT_PAGE_SIZE))
                .build();
            String body = execute(token, requestBuilder(url).get().build(), "列出 PR 评论");
    ...
    private HttpUrl issueCommentsUrl(GitRepositoryCoordinates repository, int prNumber, String token)
    {
    ...
    private static void validate(GitRepositoryCoordinates repository, String token, int prNumber)
    {
        ...
        {
            throw new GitPullRequestCommentException("Gitee 凭据不可用，无法投递评论");
        }
        if (prNumber <= 0)
        {
