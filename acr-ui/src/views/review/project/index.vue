<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="900px" top="5vh" append-to-body
      ...
      <el-form ref="projectRef" :model="form" :rules="rules" label-width="110px" class="project-form">
        ...
        <el-divider content-position="left">仓库接入</el-divider>
        ...
        <el-divider content-position="left">业务归属</el-divider>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="业务系统" prop="businessSystemId">
              <el-select v-model="form.businessSystemId" filterable style="width: 100%" placeholder="请选择业务系统" @change="handleSystemChange">
                <el-option v-for="item in options.businessSystems" :key="item.id" :label="item.label" :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="所属部门" prop="deptId">
              <el-select v-model="form.deptId" filterable disabled style="width: 100%" placeholder="选择业务系统后自动带出">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="项目负责人" prop="ownerUserId">
              <el-select v-model="form.ownerUserId" filterable style="width: 100%" placeholder="请选择负责人">
        ...
        <el-divider content-position="left">PR 审查范围</el-divider>
        <el-form-item label="启用 PR 审查" prop="prReviewEnabled">
          <div>
            ...
            <div class="inline-tip">MVP 仅审查 Pull Request，不启用 Push 审查。</div>
          </div>
        </el-form-item>
        <el-form-item v-if="form.prReviewEnabled === '0'" label="目标分支" prop="prTargetBranches">
          <div class="branch-field">
            <el-select v-model="form.prTargetBranches" multiple filterable collapse-tags :max-collapse-tags="2"
              style="width: 100%" :placeholder="repositoryInfoLoaded ? '请选择 PR 目标分支' : '请先读取仓库信息'">
              <el-option v-for="branch in branchOptions" :key="branch" :label="branch" :value="branch" />
            </el-select>
            <div class="branch-actions">
              <span class="inline-tip">来源分支默认全部允许，通常只选择 dev 或 develop，避免后续合并重复审查。</span>
              <div>
...
const options = reactive({
  businessSystems: [], departments: [], owners: [], credentials: [],
  longLivedBranches: [], robotBranchPrefixes: [], prEvents: []
})
...
.project-form { max-height: calc(90vh - 180px); overflow-y: auto; padding-right: 12px; }
.project-form :deep(.el-divider__text) { color: var(--el-text-color-primary); font-size: 14px; font-weight: 600; }
.credential-field { width: 100%; }
.credential-row { display: flex; align-items: center; gap: 10px; width: 100%; }
.credential-row .el-select { flex: 1; min-width: 220px; }
.repository-summary {
  display: grid;
...
.repository-summary strong { overflow: hidden; color: var(--el-text-color-regular); font-weight: 500; text-overflow: ellipsis; white-space: nowrap; }
.inline-tip { margin-top: 4px; color: var(--el-text-color-secondary); font-size: 12px; line-height: 20px; }
.inline-tip.is-error { display: flex; align-items: center; gap: 6px; color: var(--el-color-danger); }
.branch-field { width: 100%; }
.branch-actions { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; }
.branch-actions .inline-tip { flex: 1; }
.branch-actions > div:last-child { flex: 0 0 auto; white-space: nowrap; }
.advanced-settings { margin: -4px 0 4px 110px; border-top: 0; }
.advanced-settings :deep(.el-collapse-item__header) { height: 36px; color: var(--el-text-color-secondary); font-size: 13px; }
...
@media (max-width: 900px) {
  ...
  .credential-row .el-select { flex-basis: 100%; }
