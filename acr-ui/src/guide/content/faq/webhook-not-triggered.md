...
- **有投递但响应非 2xx 或超时**：回调地址不可达或配置错误，跳第 3、4 步。
...
| Webhook 载荷超过大小限制 | 默认上限 256KB，超大载荷需联系管理员调整后端参数 `review.webhook.max-payload-bytes` |
| 重复投递，已忽略 | 平台幂等保护，同一事件不会重复建任务 |
...
- 用 GitHub 的 **Test delivery** 测试：它发送的是 push 样例载荷，平台不会建任务（可能记录为「载荷非法」或「非合并请求事件」），验证请以真实 PR 为准；
