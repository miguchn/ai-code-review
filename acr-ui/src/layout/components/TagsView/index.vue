<template>
  <div id="tags-view-container" class="tags-view-container" :class="{ 'tags-view-container--chrome': tagsViewStyle === 'chrome' }">
    ...
    <!-- 右键上下文菜单：Teleport 避免被标签栏 overflow 裁剪；延迟绑定点击关闭避免打开即关 -->
    <Teleport to="body">
      <ul
        v-show="visible"
        :style="{ left: left + 'px', top: top + 'px' }"
        class="contextmenu tags-view-contextmenu"
      >
...
let closeMenuClickTimer = null

watch(visible, (value) => {
  if (closeMenuClickTimer != null) {
    clearTimeout(closeMenuClickTimer)
    closeMenuClickTimer = null
  }
  document.body.removeEventListener('click', closeMenu)
  if (value) {
    nextTick(() => {
      closeMenuClickTimer = setTimeout(() => {
        closeMenuClickTimer = null
...
/* Teleport 到 document.body：不能使用 .tags-view-container .contextmenu，否则 scoped 样式无法命中 */
.tags-view-contextmenu.contextmenu {
  margin: 0;
  ...
  box-shadow: var(--shell-dropdown-shadow, 0 12px 40px rgba(15, 23, 42, 0.12));
