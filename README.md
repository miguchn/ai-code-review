...
>
> ⚠️ **研发阶段声明**：本项目 MVP（V0.1）功能面已收口：GitHub、GitLab、Gitee、Gitea 四平台均已实现「项目与凭据配置 → Webhook 接收 → 审查执行 → 总结评论回写 → IM 通知 → 问题处置 → 工作台」主链路；自动化测试已通过，各平台真实环境闭环待有实例时人工验收。整体仍处内部试点阶段，请勿在生产中依赖。
...
| 版本 | 产品目标 | 核心范围 |
|------|------|------|
| MVP（V0.1 内部试点） | 最小可信闭环；**功能面已收口（2026-08-04），待真实环境验收** | 四平台 MR/PR、项目接入、Webhook、双审查路径、任务/问题、总结回写、IM 三渠道通知、权限审计、失败恢复和基础工作台 |
