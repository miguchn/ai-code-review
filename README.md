...
当前仓库已具备业务系统归属管理，以及模型配置、启停、默认模型和连接测试的基础能力；P0/M1 已完成 GitHub 项目、加密 PAT 凭据、GitHub Provider、连接检测及对应权限和页面；M2 已完成 GitHub PR Webhook 验签接入、事件幂等落库和 PENDING 审查任务生成；M3 已打通任务异步执行、工作区准备、OCR 审查、结果快照与失败重试；M3.1 已将审查任务收敛为执行队列并新增审查记录历史视图（回写/通知仍未做）。一个业务系统可关联多个代码仓库项目；业务系统的代码和接口仍由 `acr-system` 提供，唯一菜单入口已移动到“代码审查”。实际完成度以产品路线图的“当前基线”为准。
