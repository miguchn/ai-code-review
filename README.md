...
| 能力 | 说明 |
|---|---|
| 🧩 四平台统一接入 | GitHub 已完成真实仓库闭环；GitLab / Gitee / Gitea 已具备统一适配、Webhook 与回写契约，待真实实例验收 |
...
| 📤 推送审查 | 不止 MR/PR：直推 main/develop 等配置分支也触发审查，问题按参考分支归组，提交上下文注入 |
| ✍️ 行内评论 | 严重/高危问题直接发布到 MR 代码行上，一问题一评论、全生命周期只发一次（项目开关 + 严重度门槛） |
| 📊 数据洞察 | 质量总览、项目明细与成员分析（提交趋势、增删行数、被审情况）；关联提交邮箱即可查看自己的数据 |
...
**诚实说明**：V0.1（MVP）已交付并通过 GitHub 真实仓库全链路验收；核心版已陆续交付 M8 问题生命周期、M8.1 台账可视化与批量处置、M9 功能助手、企业级架构风险修复（S1–S6）、M10 推送审查、M11 行内评论、M12 数据洞察与身份关联、项目级权限治理、通知结论策略/冷却频控，以及 Token 用量分析（用量采集与成本估算，配额未交付）。当前定位是 **V0.2 核心版受控推广，不等于企业正式生产准入**。

未交付的“报告中心”默认隐藏并停用；统一身份或完整账号生命周期、业务审计导出、数据留存、备份恢复演练、不可变报告与订阅、Token/成本/配额事实仍是 V1.0 企业版准入项。GitLab / Gitee / Gitea 当前仅完成适配器与契约测试，真实环境闭环待实例；前端包体与分包预算属于测量后优化项。详细真值表和风险接受见[正式上线前完整性治理](docs/planning/production-readiness-governance.md)。
