...
- 项目管理 Webhook Secret 配置旁增加「随机生成」按钮：一键生成随机 Secret，也允许用户自行填写（2026-08-04 联调发现：Secret 为用户自填项且后端强制验签，留空会导致 Webhook 全部验签失败，用户无提示容易漏填）
- IM 通知消息内容与样式重构：2026-08-04 联调实测企微消息（总分/PR 信息/范围统计平铺）观感细碎，需重新设计信息层级与排版，突出结论与重点问题；设计时确认各渠道机器人（企微/飞书/钉钉类）对 Markdown 的渲染能力差异
- 投递记录增加「消息详情」查看：列表行点击可打开本次实际推送的完整消息内容，以 Markdown 形式渲染；需先在投递链路持久化发送的消息快照（当前 `review_delivery_record` 仅记录状态与外部 ID，无正文），涉及数据对象变更，立项时补增量 SQL
