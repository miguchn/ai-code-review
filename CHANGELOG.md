...
- 台账/详情展示责任人、转派（本人或关闭权限者，目标限项目有效成员）、逾期红色标记与筛选；工作台新增「指派给我」
- 审查总结消息按渠道 @ 责任人（钉钉 atMobiles / 企微 `<@userid>` / 飞书 `<at>`，无身份降级姓名）；个人设置激活「我的 IM 账号」
- 每日逾期扫描重算标志；项目启用通知时每项目每日一条聚合提醒（幂等键 `overdue-remind:{projectId}:{yyyyMMdd}`，不受冷却约束）
- 设计事实源 `docs/planning/issue-auto-assignment-m13.md`；三渠道真实群 @ 待人工验证

### Token 用量分析（sql/48）

- run 级用量采集：LLM 直审全部返回路径（含失败）解析响应 usage 落库（input/output/total_tokens，字段别名兼容，采集旁路不影响审查主流程）；OCR 引擎尽力读取输出 usage/tokens 节点，缺失由数据缺口指标如实展示；采集上线前历史不回填
- 成本估算：模型配置增输入/输出单价（元/千 token），读取时按当前单价计算，任一单价缺失显示「—」绝不以估算值冒充；记录详情页补 Token 用量行
- 数据洞察二级菜单「Token 用量分析」（`insight:token:view`，角色 2/4/5/6）：KPI + 趋势 + 模型占比环图表 + 项目排行表 + 审查明细表，单页联动下钻（模型/项目行点击设筛选，明细行跳记录详情）；数据范围同洞察（DataScope × 项目成员交集），直查 `review_task_run` 不建聚合表（`idx_run_token_time` 索引）
- 设计事实源 `docs/planning/token-usage-analysis.md`；活体采集 E2E 待真实 webhook 触发验证

### 数据洞察 UI 一致性

- 项目/成员分析接入平台标准搜索栏与工具栏：筛选表单 `v-show` 显隐 + 查询/重置成对 + right-toolbar（搜索显隐/刷新）
- 成员分析重排为标准三段式：两视图筛选合并为一个标准表单（时间范围共用，团队独有条件按视图显隐）；本人/团队视图切换器移入操作行（导航级操作不随搜索栏折叠）；合规声明降为 info 语义并下移贴数据区；行为零变化（本人视图即时刷新/团队视图点查询生效均保留）
...
- 审查成功对账后将 `issueId` 固化到运行的 `top_issues_json` 展示快照，后续记录追溯不依赖问题当前指纹；历史快照继续通过既有指纹兼容关联，无新表、无新状态、无新权限串
- 问题台账增加审查记录、分支、PR 发起人、发现时间筛选；列表明确“问题 #ID”“变更来源”“首次/最近审查记录”“发现/最近变更时间”，汇总“需处理”调整为“待人工处置（待确认 + 待复核）”
- 问题详情补齐项目/业务系统、PR 或 Push、发起人、分支、提交、发现/阶段/关闭信息，拆分首次发现与最近审查记录；待复核明确提示“疑似已修复、待人工验证”，时间线区分系统事件与人工处置
- 新增 `GET /review/issue/record/{taskId}`，沿用 `review:issue:list` 与项目部门数据范围；详情接口兼容保留 `sourceTask`，同时返回 `firstTask` / `lastTask`
- 设计文档：`docs/planning/issue-record-context-p0-p1.md`；后端全量 508 例测试、全模块编译和前端生产构建通过
...
- 【后续立项】问题协同治理：负责人/认领/转派、整改时限、逾期提醒与外部工单同步需要统一的指派记录、权限、通知和审计闭环；不在问题表上孤立增加负责人字段
- 【后续立项】严重级别差异化默认策略（如 LOW 级仅通知）、结合复审证据的智能预判，以及审计级全量任务-问题快照；现有 M8/M8.1/M8.2 已完成自动复核、批量处置、生命周期可视化和审查记录精确上下文
